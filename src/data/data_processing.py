...
def install_spacy_dependancy():
    ...
    if "en_core_web_sm" not in spacy.util.get_installed_models():
        logger.info("spaCy model not found. Installing 'en_core_web_sm' model...")
...
def preprocess_text_spacy(text):
    ...
    logger.info("Preprocessing text data with spaCy...")
...
def vectorize_text_using_word2vec(df: pd.DataFrame, model: Word2Vec) -> pd.DataFrame:
    ...
    logger.info(f"Word2Vec vectorization completed. Dataframe shape: {df.shape}")
    return df
...
def processing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Vectorize the data
    logger.info("Step 3: Starting vectorization process")
...
def main():
    ...
    logger.info("Step 1: Data extraction from CSV file")
    ...
    df['account_encoded'] = df['account'].replace({'Cash': 1, 'Bank': 0})
    df['type_encoded'] = df['type'].replace({'Expense': 1, 'Income': 0})
    df.drop(columns=['account','type'],inplace=True)
    df = processing(df)
